...
namespace Engine {
    Cylinder::Cylinder(std::vector<GL::Program*>& programs, float radius, float height, int segments) :
        Object(programs),
        mRadius(radius),
        mHeight(height),
        mSegments(segments)
    {
        ...
        mNormals.push_back(glm::vec3(0.0f, 1.0f, 0.0f));  // Нормаль верхнего центра
        mNormals.push_back(glm::vec3(0.0f, -1.0f, 0.0f)); // Нормаль нижнего центра
        ...
        for (int i = 0; i <= segments; ++i) {
            ...
            // Нормали для боковых точек
            glm::vec3 sideNormal = glm::normalize(glm::vec3(x, 0.0f, z));
            mNormals.push_back(sideNormal); // Верхняя боковая точка
            mNormals.push_back(sideNormal); // Нижняя боковая точка
        }
